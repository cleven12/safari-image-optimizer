 ...
 * @param {boolean} preserveStructure - Whether to preserve subdirectory structure
 * @param {string} baseDir - Base directory for relative path calculation
 * @returns {Promise<Array<{inputPath: string, relativeDir: string, filename: string, ext: string}>>}
 */
 ...
 * @param {number} wmWidth - Watermark width
 * @param {number} wmHeight - Watermark height
 * @returns {{left: number, top: number}}
 */
function getWatermarkPosition(position, imgWidth, imgHeight, wmWidth, wmHeight) {
  const padding = Math.round(Math.min(imgWidth, imgHeight) * 0.03);
 ...
 * @param {Object} file - File object from getImageFiles
 * @param {Object} config - Processing configuration
 * @param {Buffer} watermarkBuffer - Resized watermark buffer
 * @param {cliProgress.SingleBar} bar - Progress bar instance
...
async function optimizeSingle(file, config, watermarkBuffer, bar) {
  try {
    ...
    // Format-specific output with quality fallback
    let outputBuffer;
    ...
    while (finalBuffer.length > config.targetSizeKB * 1024 && finalQuality > 30) {
      ...
      const fallbackPipeline = sharp(resizedBuffer.data).composite([
        {
          input: wmResized,
          left: pos.left,
          top: pos.top,
      ...
      if (config.format === 'jpeg') {
        finalBuffer = await fallbackPipeline
          .jpeg({ quality: finalQuality, progressive: true, mozjpeg: true })
          .toBuffer();
      } else if (config.format === 'webp') {
        finalBuffer = await fallbackPipeline.webp({ quality: finalQuality }).toBuffer();
      } else if (config.format === 'avif') {
        finalBuffer = await fallbackPipeline.avif({ quality: finalQuality }).toBuffer();
      }
    }
 ...
 * @param {Object} config - Full processing configuration
 * @returns {Promise<{total: number, success: Array, failed: Array}>}
 */
async function processImages(config) {
  ...
  console.log(
    chalk.blue(
      `🎯 Target: max ${config.maxWidth}px width, ~${config.targetSizeKB}KB, ${config.format.toUpperCase()}, ${config.quality}% quality`
    )
  );
  console.log(
    chalk.blue(
      `💧 Watermark: ${config.watermarkPosition}, ${config.watermarkScale * 100}% width, ${
  ...
  const bar = new cliProgress.SingleBar(
    {
      format:
        '   ' +
        chalk.cyan('{bar}') +
        ' {percentage}% | {value}/{total} images | {filename}',
      barCompleteChar: '█',
      barIncompleteChar: '░',
      hideCursor: true,
    },
...
module.exports = { processImages, getImageFiles, getWatermarkPosition, optimizeSingle };
